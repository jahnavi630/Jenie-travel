  ...
  return (
    <div>
      <form onSubmit={handleSubmit} className="space-y-2">
        ...
        <Button type="submit" disabled={isLoading || !destination || !budget || !days}>
      ...
      {savingTips.length > 0 && (
        <div className="tool-result mt-4">
          <div className="flex justify-between items-center mb-3">
            <h4 className="tool-heading">Budget Optimization Tips:</h4>
          ...
          {savingTips.map((tip, index) => (
            <div key={index} className="mb-3 pb-2 border-b border-gray-100">
              <div className="flex justify-between">
                <span className="font-medium text-purple-700">{tip.category}</span>
                <span className="text-green-600 font-medium">-{formatCurrency(tip.saving)}</span>
              </div>
